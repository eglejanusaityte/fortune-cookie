...
public class FortuneCookieServiceImpl implements FortuneCookieService {
    ...
    public FortuneCookie createFortuneCookie(String username) {
        ...
        List<WordDAO> nonPersonalWords = allWords.stream().filter(word -> !word.getPersonal()).toList();
    ...
    @Override
    public FortuneCookieDTO createFortuneCookieShort(String username) {
        ...
        return new FortuneCookieDTO(fortuneCookie.getId(), username, sentence, fortuneCookie.getDate(), personal, usernames);
    }
    ...
    private Set<WordDAO> getRandomSubset(List<WordDAO> allWords, Set<NeededWordDAO> neededWords) {
        ...
        for (NeededWordDAO neededWord : neededWords) {
            List<WordDAO> matchingWords = allWords.stream().filter(word -> word.getWordType().equals(neededWord.getWordType())).toList();
    ...
    public FortuneCookieDTO createFortuneCookiePersonal(Long fortuneId, String username, List<String> words) {
        Optional<UserDAO> user = Optional.ofNullable(userRepository.findByUsername(username));
        UserDAO userDAO = user.orElseThrow();

        FortuneDAO fortuneDAO = fortuneRepository.findById(fortuneId).orElseThrow(() -> new NoSuchElementException("Fortune not found"));
        ...
        return new FortuneCookieDTO(savedFortuneCookieDAO.getId(), username, sentence, savedFortuneCookieDAO.getDate(), true, new HashSet<>());
    }

    public Page<FortuneCookieDTO> getFortuneCookiesByUserId(Integer page, String username) {
        Optional<UserDAO> user = Optional.ofNullable(userRepository.findByUsername(username));
        UserDAO userDAO = user.orElseThrow();
        ...
        List<FortuneCookieDTO> modifiedFortuneCookies = new ArrayList<>();
        for (FortuneCookieDAO fortuneCookie : fortuneCookieDAOs) {
            ...
            Set<String> usernames = fortuneCookie.getLikes().stream().map(UserDAO::getUsername).collect(Collectors.toSet());
            FortuneCookieDTO dto = new FortuneCookieDTO(fortuneCookie.getId(), username, sentence, fortuneCookie.getDate(), personal, usernames);
    ...
    public Page<FortuneCookieDTO> getFortuneCookies(Integer page) {
        LocalDate today = LocalDate.now();
        ...
        List<FortuneCookieDTO> modifiedFortuneCookies = fortuneCookieDAOs.stream().map(fortuneCookie -> {
            String sentence = fortuneCookie.getFortuneDAO().getSentence();
            boolean personal = false;
            for (WordDAO word : fortuneCookie.getWords()) {
                String placeholder = "#" + word.getWordType();
            ...
            Set<String> usernames = fortuneCookie.getLikes().stream().map(UserDAO::getUsername).collect(Collectors.toSet());
            return new FortuneCookieDTO(fortuneCookie.getId(), fortuneCookie.getUserDAO().getUsername(), sentence, fortuneCookie.getDate(), personal, usernames);
    ...
    public void deleteFortuneCookie(Long fortuneCookieId, String username) {
        FortuneCookieDAO fortuneCookieDAO = fortuneCookieRepository.findById(fortuneCookieId).orElseThrow(() -> new NoSuchElementException("Fortune cookie not found"));
        Optional<UserDAO> user = Optional.ofNullable(userRepository.findByUsername(username));
        UserDAO userDAO = user.orElseThrow();
